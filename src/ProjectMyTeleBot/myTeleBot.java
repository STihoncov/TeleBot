...
public class myTeleBot {
    public static void main(String[] args) throws IOException {
        TelegramBot bot = new TelegramBot("TokenBot");
        inputNumberInfo(bot);
            ...
            updates.forEach(upd -> {
                try {
                    ...
                    switch (incomeMessage) {
                        case "1" -> messageValute(bot, chatId);
                        case "2" -> messageNASAfoto(bot, chatId);
                        case "3" -> messageFunStory(bot, chatId);
                    }
                }catch (Exception e) {
                    e.printStackTrace();
                }
